...
function v(e, t, n = /* @__PURE__ */ new Map()) {
  ...
  if (l.isArrayLiteralExpression(e))
    return [e.elements.flatMap((i) => v(i, t, n))];
  if (l.isObjectLiteralExpression(e)) {
    ...
    for (const s of e.properties) {
      ...
      const c = B(s.name);
      c && (i[c] = l.isShorthandPropertyAssignment(s) ? t.get(s.name.text) || [x] : v(s.initializer, t, n));
    }
    return [i];
  }
  if (l.isIdentifier(e)) return n.get(e.text) || t.get(e.text) || [x];
  if (l.isPropertyAccessExpression(e)) {
    const i = [];
    for (const s of v(e.expression, t, n).flat(1 / 0))
      s && s !== x && typeof s == "object" ? i.push(...s[e.name.text] || [x]) : i.push(x);
    return i;
  }
  ...
  ) ? [...v(e.left, t, n), ...v(e.right, t, n)] : l.isCallExpression(e) && l.isIdentifier(e.expression) && e.expression.text === "defineSvgIcons" ? v(e.arguments[0], t, n) : [x];
}
function j(e) {
  var n, i, s, c;
  ...
  for (const i of n.statements)
    if (l.isVariableStatement(i))
      for (const s of i.declarationList.declarations)
        l.isIdentifier(s.name) && s.initializer && t.set(s.name.text, v(s.initializer, t));
...
function D(e, t, n) {
  ...
  function s(c) {
    if (l.isCallExpression(c) && l.isIdentifier(c.expression) && c.expression.text === "defineSvgIcons") {
      ...
      for (const p of F(v(c.arguments[0], t)))
...
function k(e, t, n) {
  ...
  if (!i) return;
  const s = (i[1] || i[2]).trim(), c = j(i[3]);
  n.set(s, v(c, t, n).flat(1 / 0));
}
function K(e, t, n) {
  const i = [], s = [], c = z(e);
  function u(f, d) {
    ...
    if (f.type === 1) {
      ...
      if ((y = o == null ? void 0 : o.exp) != null && y.content && k(o.exp.content, t, g), ["svg-icon", "SvgIcon"].includes(f.tag)) {
        const a = f.props.find((m) => {
          var w;
          return m.type === 6 ? ["icon-class", "iconClass"].includes(m.name) : m.type === 7 && m.name === "bind" && ["icon-class", "iconClass"].includes((w = m.arg) == null ? void 0 : w.content);
        });
        if ((a == null ? void 0 : a.type) === 6 && (($ = a.value) != null && $.content) && u([a.value.content], a.loc.start.line), (a == null ? void 0 : a.type) === 7 && ((r = a.exp) != null && r.content)) {
          const m = v(j(a.exp.content), t, g);
          u(m, a.loc.start.line), m.flat(1 / 0).includes(x) && s.push({
            file: n,
...
function W(e, t) {
  var f, d, g;
  if (!e.endsWith(".vue")) {
    const { env: y, sourceFile: $ } = _(t), r = D($, y, e);
...
`), { env: s, sourceFile: c } = _(i), u = K(((g = n.template) == null ? void 0 : g.content) || "", s, e), p = D(c, s, e);
...
async function H(e) {
  ...
  for (const s of e) {
    const c = await T("**/*.svg", { cwd: s, absolute: !0 });
...
const L = "virtual:svg-icons-register", P = "virtual:svg-icons-names", A = `\0${L}`, N = `\0${P}`, M = "__svg__icons__dom__";
function te(e) {
  return `
...
function fe(e) {
  ...
  async function f() {
    ...
    for (; a.length; ) {
      ...
      if (o.has(m)) continue;
      o.add(m);
      const w = await E.readFile(m, "utf8");
      for (const I of G(w)) {
  ...
  async function d() {
    ...
    for (const w of r.sort()) {
      const I = await E.readFile(w, "utf8"), S = W(w, I);
  ...
  return {
    ...
    configResolved(r) {
      ...
      c = Array.isArray(o) ? o : Object.entries(o).map(([m, w]) => ({ find: m, replacement: w }));
    },
    async buildStart() {
      await g();
    },
